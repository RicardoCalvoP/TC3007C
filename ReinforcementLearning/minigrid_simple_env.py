...
def q_learning():
    ...
    Q_table = np.zeros((size, size, 4, 3)) # Initialize Q-table: Q[x][y][direction][action]
    ...
    max_iterations = 50                    # Set the number of episodes to run before stopping
    max_trainings = 22124                    # Max number of training sets
    eps_start = 1.0                        # Starting value of epsilon for epsilon-greedy action selection
    eps_end = 0.05                         # Final value of epsilon
    eps_decay = 500                        # Decay rate of epsilon
    ...
    for j in trange(max_trainings, desc="Training",  colour="cyan", ncols=100):
        ...
        while step < (max_iterations) and not (terminated or truncated):
